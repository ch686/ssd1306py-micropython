...
class SSD1306(framebuf.FrameBuffer):
	...
	def set_myfont(self,font, font_size):
		"""设置字库"""	
		if font_size not in [8,16,24,32]:
			return
	...
	def draw_text(self,string, x_axis, y_axis, font_size=8):
		...
		for k in string:			
			...
			byte_data=list(bytearray(font_size//8*font_size))			
			...
			if font_size in self._fonts:
				if code in self._fonts[font_size]:
			...
			if len(data_code)==3: #中文				
				...
				if byte_data[0]==0xaa and byte_data[1]==0x55 and font_size==8:
					self.text(k, x_axis+ offset,y_axis)
					offset += font_size
				else:	
